  ...
  publish: async (req, res, next) => {
    try {
      ...
      const getCityId = await cities.findOne({
        where: { city: req.body.event.eventCity }, raw: true
      },)
      ...
      const getEventId = await events.findOne({ where: { name: req.body.event.eventTitle }, raw: true })
      ...
      next(templateRes(201, true, "create event, ticket, promo success", [createEvent, createTicket], null))
    } catch (error) {
